...
public class KeyHandler {
	...
	public static boolean addClientKeyBind(IKeyBinding binding) {
		...
		if (!clientBinds.containsKey(binding.getID())) {
			keys.add(binding);
			clientBinds.put(binding.getID(), binding);
			return true;
		}
		return false;
	}
	...
	public static boolean addServerKeyBind(IKeyBinding binding) {
		...
		if (!serverBinds.containsKey(binding.getID())) {
			serverBinds.put(binding.getID(), binding);
			return true;
		}
		return false;
	}
		...
		for (IKeyBinding key : keys) {
			...
			if (press > 0 && isKeyDown(press)) {
				if (key.keyPressClient() && key.hasServerSide()) {
					PacketKey.sendToServer(key.getID());
				}
			}
		}
	}
