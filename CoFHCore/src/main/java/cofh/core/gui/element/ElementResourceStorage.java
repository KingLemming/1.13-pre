...
public abstract class ElementResourceStorage extends ElementBase {
	...
	public void addTooltip(List<String> tooltip, int mouseX, int mouseY) {
		...
		if (infinite) {
			...
			tooltip.add(formatNumber(storage.getStored()) + " / " + formatNumber(storage.getMaxStored()) + " " + storage.getUnit());
		}
	}

	protected int getScaled(int scale) {

		if (storage.getMaxStored() <= 0 || infinite) {
			return scale;
		}
		double fraction = (double) storage.getStored() * scale / storage.getMaxStored();
