...
import static cofh.lib.util.helpers.StringHelper.isAltKeyDown;
...
public class ElementTextField extends ElementBase {
	...
	public ElementTextField(IGuiAccess gui, int posX, int posY, int width, int height) {

		this(gui, posX, posY, width, height, (short) 32);
	}

	public ElementTextField(IGuiAccess gui, int posX, int posY, int width, int height, short limit) {
	...
	public int getVisibleHeight() {
		...
		return Math.min(height - renderStartY, height);
	}
	...
	public int getVisibleWidth() {
		...
		FontRenderer font = getFontRenderer();
		int width = 0, endX = width - 1, maxWidth = 0;
		if (multiline) {
			for (int i = 0; i < textLength; ++i) {
				char c = text[i];
	...
	protected void findRenderStartML() {
		...
		int pos = Math.max(0, (height - 2) / font.FONT_HEIGHT) * font.FONT_HEIGHT;
			...
			default:
				switch (keyTyped) {
					...
					case Keyboard.KEY_RIGHT: // right arrow
						...
						} else if (isAltKeyDown() && GuiScreen.isShiftKeyDown()) {
	...
	public void drawBackground(int mouseX, int mouseY) {
		...
		drawColoredModalRect(posX, posY, posX + width, posY + height, enabled() ? backgroundColor : disabledColor);
	}
	...
	public void drawForeground(int mouseX, int mouseY) {
		...
		if (enableStencil) {
			...
			RenderHelper.drawStencil(posX + 1, posY + 1, posX + width - 1, posY + height - 1, 1 << bit);
		}

		FontRenderer font = getFontRenderer();
		char[] text = this.text;
		int startX = posX + 1 - (multiline ? renderStartX : 0), endX = width - 1;
		int startY = posY + 1 - renderStartY, endY = startY + font.FONT_HEIGHT;
		int drawY = renderStartY + Math.max(0, (height - 2) / font.FONT_HEIGHT) * font.FONT_HEIGHT;
		if (enableStencil && height - (drawY - renderStartY) > 2) {
		...
		for (int i = multiline ? 0 : renderStartX, width = 0, height = 0; i <= textLength; ++i) {
			...
			if (drawCaret) {
				...
				drawColoredModalRect(startX + width, startY - 1 + height, startX + caretEnd, endY + height, (0xFF000000 & defaultCaretColor) | (~defaultCaretColor & 0xFFFFFF));
			}

			if (draw && !end) {
				boolean selected = i >= selectionStart & i < selectionEnd;
				if (selected) {
					drawColoredModalRect(startX + width, startY + height, startX + width + charW, endY + height, selectedLineColor);
				}
				if (c != '\n') {
