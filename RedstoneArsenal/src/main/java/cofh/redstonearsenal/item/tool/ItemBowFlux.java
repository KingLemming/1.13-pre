...
import cofh.core.item.tool.ItemBowCoFH;
...
public class ItemBowFlux extends ItemBowCoFH implements IEnergyContainerItem, IMultiModeItem {
	...
	public ItemBowFlux(Item.ToolMaterial toolMaterial) {
		...
		addPropertyOverride(new ResourceLocation("charged"), (stack, world, entity) -> ItemBowFlux.this.getEnergyStored(stack) > 0 && !ItemBowFlux.this.isEmpowered(stack) ? 1F : 0F);
		addPropertyOverride(new ResourceLocation("empowered"), (stack, world, entity) -> ItemBowFlux.this.isEmpowered(stack) ? 1F : 0F);
	}

	public ItemBowFlux setEnergyParams(int maxEnergy, int maxTransfer, int energyPerUse, int energyPerUseEmpowered) {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		...
		tooltip.add(ORANGE + getEnergyPerUse(stack) + " " + localize("info.redstonearsenal.tool.energyPerUse") + StringHelper.END);
	...
	public float getArrowDamageMultiplier(ItemStack item) {
		...
		return isEmpowered(item) ? damageMultiplier * 1.5F : damageMultiplier;
	}

	@Override
	public float getArrowSpeedMultiplier(ItemStack item) {

		return isEmpowered(item) ? speedMultiplier + 0.1F : speedMultiplier;
	}
	// endregion
	...
	public void generateModelFiles() {
		...
		String modelBase = "{\"parent\":\"" + domain + ":item/tools/bow\",\"textures\":{\"layer0\":\"" + domain + ":items/tools/" + path + "\"},\"overrides\":[{\"predicate\":{\"charged\":1},\"model\":\"" + domain + ":item/tools/" + path + "_charged\"},{\"predicate\":{\"pulling\":1,\"charged\":1},\"model\":\"" + domain + ":item/tools/" + path + "_charged_0\"},{\"predicate\":{\"pulling\":1,\"pull\":0.65,\"charged\":1},\"model\":\"" + domain + ":item/tools/" + path + "_charged_1\"},{\"predicate\":{\"pulling\":1,\"pull\":0.9,\"charged\":1},\"model\":\"" + domain + ":item/tools/" + path + "_charged_2\"},{\"predicate\":{\"empowered\":1},\"model\":\"" + domain + ":item/tools/" + path + "_empowered\"},{\"predicate\":{\"pulling\":1,\"empowered\":1},\"model\":\"" + domain + ":item/tools/" + path + "_empowered_0\"},{\"predicate\":{\"pulling\":1,\"pull\":0.65,\"empowered\":1},\"model\":\"" + domain + ":item/tools/" + path + "_empowered_1\"},{\"predicate\":{\"pulling\":1,\"pull\":0.9,\"empowered\":1},\"model\":\"" + domain + ":item/tools/" + path + "_empowered_2\"}]}";
		...
		for (int i = 0; i < 3; i++) {
			...
			String modelSubCharged = "{\"parent\":\"" + domain + ":item/tools/" + path + "\",\"textures\":{\"layer0\":\"" + domain + ":items/tools/" + path + "_charged_" + i + "\"}}";
			String modelSubEmpowered = "{\"parent\":\"" + domain + ":item/tools/" + path + "\",\"textures\":{\"layer0\":\"" + domain + ":items/tools/" + path + "_empowered_" + i + "\"}}";
			...
			try {
				...
				File itemModelSubCharged = new File(configDir, "/dev/" + domain + "/models/item/tools/" + path + "_charged_" + i + ".json");
				FileUtils.writeStringToFile(itemModelSubCharged, Utils.createPrettyJSON(modelSubCharged), Charset.forName("UTF-8"));

				File itemModelSubEmpowered = new File(configDir, "/dev/" + domain + "/models/item/tools/" + path + "_empowered_" + i + ".json");
				FileUtils.writeStringToFile(itemModelSubEmpowered, Utils.createPrettyJSON(modelSubEmpowered), Charset.forName("UTF-8"));
			} catch (Throwable t) {
				// pokemon!
			}
		}
