...
import static cofh.redstonearsenal.init.ConfigRSA.explosionsDestroyBlocks;
...
public class EntityArrowFlux extends EntityArrow {
	...
	public static final String NAME = "flux_arrow";
	...
	public static void initialize(int id) {
		...
		ForgeRegistries.ENTITIES.register(EntityEntryBuilder.create()//
				.entity(EntityArrowFlux.class)//
				.id(new ResourceLocation(ID_REDSTONE_ARSENAL + ":" + NAME), id)//
				.name(ID_REDSTONE_ARSENAL + "." + NAME)//
	...
	protected void onHit(RayTraceResult traceResult) {
		...
		if (!world.isRemote && empowered) {
			world.createExplosion(shootingEntity, posX, posY, posZ, EXPLOSION_STRENGTH + (getIsCritical() ? 2 + rand.nextFloat() : 0), explosionsDestroyBlocks);
		}
		this.setDead();
	}
	...
	public void onUpdate() {
		...
		if (this.ticksInAir >= MAX_TICKS) {
			if (!world.isRemote && empowered) {
				world.createExplosion(this, posX, posY, posZ, EXPLOSION_STRENGTH + (getIsCritical() ? 2 + rand.nextFloat() : 0), explosionsDestroyBlocks);
			}
			this.setDead();
		}
	}
	...
	protected Entity findEntityOnPath(Vec3d start, Vec3d end) {
		...
		return Utils.isClientWorld(world) ? null : super.findEntityOnPath(start, end);
	}
