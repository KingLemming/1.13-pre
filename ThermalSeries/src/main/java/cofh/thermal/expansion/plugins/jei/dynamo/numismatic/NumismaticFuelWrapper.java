...
public class NumismaticFuelWrapper extends BaseRecipeWrapper {

	protected List<List<ItemStack>> input;

	protected IDrawableAnimated duration;

	public NumismaticFuelWrapper(IGuiHelper guiHelper, IDynamoFuel fuel) {

		List<ItemStack> fuelInput = new ArrayList<>();
		int oreID = NumismaticFuelManager.instance().convertInput(fuel.getInputItems().get(0)).oreID;
		if (oreID != -1) {
