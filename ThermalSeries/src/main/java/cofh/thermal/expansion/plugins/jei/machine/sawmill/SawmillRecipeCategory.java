package cofh.thermal.expansion.plugins.jei.machine.sawmill;
...
import cofh.thermal.expansion.gui.client.machine.process.GuiMachineSawmill;
...
public class SawmillRecipeCategory extends BaseRecipeCategory<SawmillRecipeWrapper> {
	...
	public SawmillRecipeCategory(String uId) {
	...
	public void registerCategory(IRecipeCategoryRegistration registry) {
		...
		enable = ThermalSeries.configClient.getBoolean("Machines.Sawmill", category, enable, comment);
		...
		background = guiHelper.createDrawable(GuiMachineSawmill.TEXTURE, 26, 11, 124, 62, 0, 0, 16, 24);
		energyMeter = Drawables.getDrawables(guiHelper).getEnergyEmpty();
		localizedName = StringHelper.localize("tile.thermal.machine_sawmill.name");
	...
	public void registerRecipes(IModRegistry registry) {
		...
		registry.addRecipeClickArea(GuiMachineSawmill.class, 72, 34, 24, 16, uId);
		registry.addRecipeCatalyst(BlocksTE.machineSawmill, uId);
	}
	...
	public static List<SawmillRecipeWrapper> getRecipes(IGuiHelper guiHelper) {
		...
		for (IMachineRecipe recipe : SawmillRecipeManager.instance().getRecipeList()) {
