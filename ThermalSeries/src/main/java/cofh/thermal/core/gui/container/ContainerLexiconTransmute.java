...
import static cofh.lib.util.oredict.OreDictHelper.hasOreName;
import static cofh.thermal.core.network.PacketLexiconTransmute.*;

public class ContainerLexiconTransmute extends Container {

	ArrayList<ItemStack> oreList;
	ArrayList<String> nameList;
	int oreSelection = -1;
	int nameSelection = -1;
	String oreName = ORE_UNKNOWN;
	...
	public ContainerLexiconTransmute(InventoryPlayer inventory) {
		...
		addSlotToContainer(new Slot(lexiconInv, 0, 59, 61) {
			@Override
			public boolean isItemValid(ItemStack stack) {

				return hasOreName(stack);
			}
		});
	...
	protected String getCommonOreName(ItemStack ore1, ItemStack ore2) {
	...
	protected boolean equivalentOres(ItemStack ore1, ItemStack ore2) {
	...
	public boolean doTransmute() {
		...
		oreStack = cloneStack(entry, 1);

		if (output.isEmpty()) {
			output = cloneStack(entry, input.getCount());
	...
	public void handlePacket(PacketLexiconTransmute packet) {
	...
	public void onCraftMatrixChanged(IInventory inventory) {
		...
		if (!input.isEmpty() && hasOreName(input)) {
			...
			if (!equivalentOres(input, oreStack)) {
				...
				nameSelection = 0;
				oreSelection = 0;
				oreStack = cloneStack(oreList.get(oreSelection), 1);
	...
	protected boolean mergeItemStack(ItemStack stack, int slotMin, int slotMax, boolean ascending) {
		...
		if (stack.isStackable()) {
			while (stack.getCount() > 0 && (!ascending && k < slotMax || ascending && k >= slotMin)) {
				...
				if (slot.isItemValid(stack) && itemsIdentical(stack, stackInSlot)) {
		...
		if (stack.getCount() > 0) {
			...
			while (!ascending && k < slotMax || ascending && k >= slotMin) {
				...
				if (slot.isItemValid(stack) && stackInSlot.isEmpty()) {
					slot.putStack(cloneStack(stack, Math.min(stack.getCount(), slot.getSlotStackLimit())));
					slot.onSlotChanged();
