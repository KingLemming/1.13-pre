...
public class LapidaryFuelWrapper extends BaseRecipeWrapper {

	protected List<List<ItemStack>> input;

	protected IDrawableAnimated duration;

	public LapidaryFuelWrapper(IGuiHelper guiHelper, IDynamoFuel fuel) {

		List<ItemStack> fuelInput = new ArrayList<>();
		int oreID = LapidaryFuelManager.instance().convertInput(fuel.getInputItems().get(0)).oreID;
		if (oreID != -1) {
